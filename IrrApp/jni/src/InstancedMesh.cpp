...
InstancedMesh::InstancedMesh() 
 : IsDirty(false), IsFinal(false), Box(core::vector3df(0,0,0))
{
...
bool InstancedMesh::isDirty(s32 id)
{
	if (id < 0 || id > (s32)DestBuffers.size())
		return IsDirty;
	else
...
void InstancedMesh::update()
{
	...
	for (i=0; i<DestBuffers.size(); ++i)
	{
		if (DestBuffers[i].IndexCount != DestBuffers[i].Buffer->getVertexCount() || 
...
core::array<s32> InstancedMesh::addMesh(IMesh* mesh, core::vector3df pos, core::vector3df rot, core::vector3df scale)
{
	core::matrix4 m;
...
core::array<s32> InstancedMesh::addMesh(IMesh* mesh, const core::matrix4 &transform)
{
...
s32 InstancedMesh::addMeshBuffer(IMeshBuffer* buffer, core::vector3df pos, core::vector3df rot, core::vector3df scale)
{
	core::matrix4 m;
	m.setRotationDegrees(rot);
	m.setTranslation(pos);
...
//! adds a mesh with the given transformation
/** \Return Returns the ID of this mesh buffer */
s32 InstancedMesh::addMeshBuffer(IMeshBuffer* buffer, const core::matrix4 &transform)
{
	if (!buffer || IsFinal)
		return -1;
...
void InstancedMesh::recalculateBoundingBox()
{
...
u32 InstancedMesh::getMeshBufferCount() const
{
...
IMeshBuffer* InstancedMesh::getMeshBuffer(u32 nr) const
{
...
IMeshBuffer* InstancedMesh::getMeshBuffer( const video::SMaterial &material) const 
{ 
	return 0; 
}
...
const core::aabbox3d<f32>& InstancedMesh::getBoundingBox() const
{
	return Box;
}

//! set user axis aligned bounding box
void InstancedMesh::setBoundingBox( const core::aabbox3df& box)
{
	Box = box;
}
...
void InstancedMesh::setMaterialFlag(video::E_MATERIAL_FLAG flag, bool newvalue)
{
	u32 i;
	for (i=0; i<DestBuffers.size(); ++i)
...
//! Moves a mesh
core::array<bool> InstancedMesh::moveMesh(const core::array<s32>& bufferIDs, const core::matrix4 &newMatrix)
{
	core::array<bool> result;
	u32 i;
	for (i=0; i<bufferIDs.size(); ++i)
		moveMeshBuffer(bufferIDs[i], newMatrix);

	return result;
}

//! Moves a mesh buffer
bool InstancedMesh::moveMeshBuffer(const s32 id, const core::matrix4 &newMatrix)
{
	if (id<0 || id > (s32)BufferReferences.size() || IsFinal )
		return false;
...
IMeshBuffer* InstancedMesh::getSourceBuffer(s32 id)
{
	if (id<0 || id > (s32)BufferReferences.size() || IsFinal)
		return 0;
	else
...
core::matrix4 InstancedMesh::getSourceBufferMatrix(s32 id)
{
	core::matrix4 ret;
	if (id<0 || id > (s32)BufferReferences.size() || IsFinal)
		ret.makeIdentity();
...
u32 InstancedMesh::getSourceBufferCount()
{
...
void InstancedMesh::recalculateDestBufferBoundingBox(u32 i)
{
	switch (DestBuffers[i].VertexType)
	{
	case video::EVT_STANDARD:
...
void InstancedMesh::updateDestFromSourceBuffer(u32 i)
{
...
void InstancedMesh::addSourceBuffer(IMeshBuffer *source)
{
	u32 i;
	bool found = false;
	for (i=0; i<SourceBuffers.size(); ++i)
