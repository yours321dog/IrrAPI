...
#ifdef _IRR_COMPILE_WITH_ANDROID_DEVICE_11111122222222223
#include <abcdeg.h>
#endif
...
{
	//__android_log_print(ANDROID_LOG_INFO,"Enron","COGLESDriver:deleteAllDynamicLights()");
...
{
	////__android_log_print(ANDROID_LOG_INFO, "Enron", "COGLESDriver:addDynamicLight()");
...
{
	//__android_log_print(ANDROID_LOG_INFO,"Ernon","COGLESDriver:turnLightOn()"); 
...
void COGLES1Driver::drawStencilShadowVolume(const core::vector3df* triangles, s32 count, bool zfail)
{
	...
	//传过来的参数列表：ShadowVolumes[i].vertices,ShadowVolumes[i].count, UseZFailMethod
	//vertices数组中，一共存有四个三角形，每三个顶点表示一个三角形
	...
	//__android_log_print(ANDROID_LOG_INFO, "Enron", "Draw StencilVolumeFinished!");
}
...
{
	//__android_log_print(ANDROID_LOG_INFO, "Enron", "COGLESDriver:DrawStencilShaodow()");
	if (!StencilBuffer)
		return;
