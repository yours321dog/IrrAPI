...
uniform bool  uUseLi         [MAX_LIGHTS];
uniform vec4  uLiPos    [MAX_LIGHTS];
uniform vec4  uLiAmb     [MAX_LIGHTS];
uniform vec4  uLiDif     [MAX_LIGHTS];
#ifdef USE_SPECULAR
uniform vec4  uLiSpe    [MAX_LIGHTS];
#endif
#ifdef USE_LIGHT_CUTOFF
uniform vec3  uLiDir   [MAX_LIGHTS];
uniform float uLiCutoff      [MAX_LIGHTS];
...
{		
	...
	if( uLiPos[lidx].w == 0.0) // Directional light
	{
		lightDir = -uLiPos[lidx].xyz;
	}
	else
	{
		lightDir = uLiPos[lidx].xyz - inVertexPosition.xyz;
		att = 1.0 / (uLightAttenuation[lidx].y * length(lightDir));
		lightDir = normalize(lightDir);
		
#ifdef USE_LIGHT_CUTOFF
		if(uLiCutoff[lidx] < 180.0)
		{
			// compute spot factor
			float spotEffect = dot(-lightDir, uLiDir[lidx]);
			if( spotEffect >= cos( radians( uLiCutoff[lidx])))
	...
	if(att >= 0.0 )
	{
		color += uLiAmb[lidx] * gMaterial.Ambient;
		
		//Compute cos(Light, Normal)
		float NdotL = max(dot(normalize(gNormal), lightDir), 0.0);
		color += NdotL * uLiDif[lidx] * gMaterial.Diffuse;
		...
		if(NdotH > 0.0)
		{
			color += pow(NdotH, uMaterialShininess) * uLiSpe[lidx] * gMaterial.Specular;
		}
#endif
		color *= att;
	}
...
vec4 computeLighting(void)
{
	...
	//vec4 color = gMaterial.Emission + gMaterial.Diffuse;	
